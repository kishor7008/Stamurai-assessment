...
const Table: React.FC = () => {
  const headingRefs = useRef<{ [key: string]: HTMLSpanElement | null }>({});
  const [cityData, setCityData] = useState<CityData[]>([]);
  const [colorUpArrow, setColorUpArrow] = useState(false);
  ...
  const handleDescendingOrder = (head: string) => {
    ...
    setCityData(sortData);
    setColorUpArrow(false);
    setColorDownArrow(true);
  };

  const TABLE_HEAD = ["Country Code", "Countries", "City Name", "TimeZone", "Population"];
  ...
  onPointerLeaveCapture={() => console.log('Pointer left')}>
          ...
          >
            {!loading ? (
                  ...
                  {cityData.length > 0 ? (
                    cityData.map(
                      ({ country_code, cou_name_en, name, population, timezone }, index) => {
                        ...
                        const classes = isLast ? "p-4" : "p-4 border-b border-blue-gray-50 text-left";
                        return (
              ...
              <td
                className="px-6 py-4 col-span-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                colSpan={5}
              >
